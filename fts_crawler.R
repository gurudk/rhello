...
urls <- str_extract_all(body, pattern = "http[^\"]*fts3[^\"]*(txt|dat)") #extract urls 
for(url in urls[[1]]){
  ...
  write.csv(file_content, file_name, row.names = FALSE, col.names = TRUE, quote = FALSE) #save the frame to file
